    ...
    <div>
      ...
      <div style={{ position: 'relative', height: 260, overflow: 'hidden' }}>
        {hasPhoto ? (
          ...
          <div style={{ width: '100%', height: '100%', background: artist.bg || '#0e0e0e', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{ padding: '20px 18px' }}>
        ...
        {/* Numero lavori */}
        {artist.works > 0 && (
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '10px 16px', marginBottom: 24 }}>
            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 22, fontWeight: 400, color: '#e8e4dc' }}>{artist.works}</div>
            <div style={{ fontSize: 8, fontFamily: 'DM Mono, monospace', color: '#555', textTransform: 'uppercase', letterSpacing: '.1em' }}>{t('works-label')}</div>
          </div>
        )}
        ...
        {/* Banner owner */}
        {isOwner && (
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 10, padding: '12px 14px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 6, height: 6, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', flexShrink: 0 }} />
            <span style={{ fontSize: 11, color: '#555', fontFamily: 'DM Mono, monospace', letterSpacing: '.06em' }}>
              Per modificare apri il menu → Profilo artista
            </span>
          </div>
        )}
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            <div style={{ fontSize: 8, fontFamily: 'DM Mono, monospace', color: '#444', textTransform: 'uppercase', letterSpacing: '.15em' }}>
              {t('recent-works')}
              {workPhotos.length > 0 && (
                <span style={{ color: '#333', marginLeft: 6 }}>({workPhotos.length})</span>
              )}
            </div>
          </div>
          {workPhotos.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6 }}>
              {workPhotos.map((work, i) => (
                <div key={i} style={{ aspectRatio: '1/1', borderRadius: 8, overflow: 'hidden', background: '#1a1a1a', border: '1px solid rgba(255,255,255,0.05)' }}>
                  <img src={work.url || work} alt={'Lavoro ' + (i + 1)} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              ))}
            </div>
          )}
