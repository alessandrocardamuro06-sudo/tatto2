...
function Sidebar({ open, onClose, onNavigate, onLogout, userProfile, t, isAdmin, currentPage }) {
  useEffect(() => {
  ...
  const navItems = [
    ...
    { id: 'custom', label: t('nav-create'), icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" style={{width:18,height:18}}>
    ...
    ...(isAdmin ? [{ id: 'admin', label: 'Admin', icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" style={{width:18,height:18}}>
        <path d="M12 2L22 7v6c0 5.25-4.5 10.14-10 11.5C6.5 23.14 2 18.25 2 13V7l10-5z"/>
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 15, color: '#e8e4dc', fontWeight: 600, marginBottom: 3 }}>
            {userProfile?.name || ''}
          </div>
          <div style={{ fontSize: 10, fontFamily: 'DM Mono, monospace', color: '#333', letterSpacing: '.12em', textTransform: 'uppercase' }}>
            {userProfile?.email || ''}
          </div>
          <div style={{
        ...
        <div style={{ padding: '16px 12px', flex: 1 }}>
          <div style={{ fontSize: 8, fontFamily: 'DM Mono, monospace', color: '#222', letterSpacing: '.2em', textTransform: 'uppercase', padding: '0 12px', marginBottom: 8 }}>
            Navigazione
          </div>
          {navItems.map(item => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? '#f0ece4' : '#444',
                  cursor: 'pointer', marginBottom: 2,
                  fontFamily: 'Syne, sans-serif', fontSize: 13, fontWeight: isActive ? 600 : 400,
                  letterSpacing: '.02em', transition: 'all .2s',
                  textAlign: 'left',
                  borderLeft: isActive ? '2px solid rgba(255,255,255,0.3)' : '2px solid transparent',
                }}
              >
                <span style={{ color: isActive ? 'rgba(255,255,255,0.7)' : '#2a2a2a', transition: 'color .2s' }}>
                  {item.icon}
                </span>
                {item.label}
              </button>
          ...
          {/* Profilo */}
          <button
            onClick={() => { onNavigate('profile-user'); onClose(); }}
            style={{
              width: '100%', display: 'flex', alignItems: 'center', gap: 12,
              padding: '11px 12px', borderRadius: 10, border: 'none',
          ...
          >
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.2" style={{width:18,height:18,color:'#2a2a2a'}}>
        ...
        <div style={{ padding: '16px 24px', borderTop: '1px solid rgba(255,255,255,0.04)' }}>
          <div style={{ fontSize: 8, fontFamily: 'DM Mono, monospace', color: '#1e1e1e', letterSpacing: '.18em', textTransform: 'uppercase' }}>
            Ink Lovers Studio
          </div>
        </div>
      </div>
...
function UserProfilePage({ onBack, userProfile, t }) {
  return (
    <div style={{ padding: '20px 18px' }}>
      ...
      {[
        ...
        ['Ruolo', userProfile?.role],
        ['Membro dal', userProfile?.createdAt ? new Date(userProfile.createdAt.seconds * 1000).toLocaleDateString('it-IT', { month: 'long', year: 'numeric' }) : '—'],
      ].map(([label, value]) => (
        <div key={label} style={{
          ...
          <div style={{ fontSize: 9, fontFamily: 'DM Mono, monospace', color: '#2a2a2a', textTransform: 'uppercase', letterSpacing: '.12em' }}>
            {label}
          </div>
          <div style={{ fontSize: 13, color: '#888', fontFamily: 'Syne, sans-serif' }}>
            {value}
          </div>
        </div>
      ))}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <img
          ...
          alt="Ink Lovers"
          style={{ height: 44, objectFit: 'contain', justifySelf: 'center' }}
        />

        {/* Language switcher dx */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, justifySelf: 'end' }}>
          {['it','en','de'].map(l => (
            <button key={l} onClick={() => setLang(l)} style={{
              ...
              color: lang === l ? '#fff' : '#333', cursor: 'pointer',
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {page === 'profile-user' && <UserProfilePage onBack={() => setPage('feed')} userProfile={userProfile} t={t} />}
      </div>
      ...
      <div style={{
        ...
        display: 'flex', flexShrink: 0, padding: '2px 0 0',
      }}>
        {navItems.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => handleNavClick(item.id)} style={{
              ...
              padding: '10px 0 10px', cursor: 'pointer', position: 'relative',
